...
try:
    from codes.models.archs.dcn.deform_conv import ModulatedDeformConvPack as DCN
except ImportError:
...
class ResidualBlock_noBN(nn.Module):
    ...
    def __init__(self, nf=64):
        ...
        self.ln1 = nn.LayerNorm(batch_size)
        self.conv2 = nn.Conv2d(nf, nf, 3, 1, 1, bias=True)
        self.ln2 = nn.LayerNorm(batch_size)
...
class Predeblur_ResNet_Pyramid(nn.Module):
    def __init__(self, nf=128, HR_in=False):
        ...
        if self.HR_in:
            ...
            self.ln_first_1 = nn.LayerNorm(batch_size)
            self.conv_first_2 = nn.Conv2d(nf, nf, 3, 2, 1, bias=True)
            self.ln_first_2 = nn.LayerNorm(batch_size)
            self.conv_first_3 = nn.Conv2d(nf, nf, 3, 2, 1, bias=True)
            self.ln_first_3 = nn.LayerNorm(batch_size)
        else:
            self.conv_first = nn.Conv2d(3, nf, 3, 1, 1, bias=True)
            self.ln_first = nn.LayerNorm(batch_size)
        ...
        self.deblur_L2_ln = nn.LayerNorm(batch_size)
        self.deblur_L3_conv = nn.Conv2d(nf, nf, 3, 2, 1, bias=True)
        self.deblur_L3_ln = nn.LayerNorm(batch_size)

        self.lrelu = nn.LeakyReLU(negative_slope=0.1, inplace=True)

    def forward(self, x):
        if self.HR_in:
            out = self.conv_first_1(x)
            out = self.ln_first_1(out)
            L1_fea = self.lrelu(out)
            out = self.conv_first_2(L1_fea)
            out = self.ln_first_2(out)
            L1_fea = self.lrelu(out)
            out = self.conv_first_3(L1_fea)
            out = self.ln_first_3(out)
            L1_fea = self.lrelu(out)
        else:
            out = self.conv_first(x)
            out = self.ln_first(out)
            L1_fea = self.lrelu(out)
        out = self.deblur_L2_conv(L1_fea)
        out = self.deblur_L2_ln(out)
        L2_fea = self.lrelu(out)
        out = self.deblur_L3_conv(L2_fea)
        out = self.deblur_L2_ln(out)
        L3_fea = self.lrelu(out)
...
class EDVR(nn.Module):
    def __init__(self, nf=64, nframes=5, groups=8, front_RBs=5, back_RBs=10, center=None,
                 predeblur=False, HR_in=False, w_TSA=True, batch=None):
...
if __name__ == '__main__':
    ip = torch.randn(20,5,10,10)
    m = nn.LayerNorm(ip.size()[1:])
    output = m(ip)
    m = nn.LayerNorm(ip.size()[1:], elementwise_affine=False)
    output = m(ip)