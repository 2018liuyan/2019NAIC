...
def main():
    ...
    if rank <= 0:  # normal training (rank -1) OR distributed training (rank 0)
        ...
        if opt['use_tb_logger'] and 'debug' not in opt['name']:
            ...
            print('../tb_logger/', opt['name'])
            tb_logger = SummaryWriter(log_dir='/root/ly33/mytorch/2019NAIC-challenge/naic-2019/mmsr/experiments/'+  opt['name'])
    else:
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            if opt['datasets'].get('val', None) and current_step % opt['train']['val_freq'] == 0:
                ...
                if opt['model'] in ['sr', 'srgan'] and rank <= 0:  # image restoration validation
                    ...
                    if opt['dist']:
                        ...
                        psnr_total_avg /= len(psnr_rlt)
                        log_s = '# Validation # PSNR: {:.4e}:'.format(psnr_total_avg)
                        print('# Validation # PSNR:',psnr_total_avg)
                        for k, v in psnr_rlt_avg.items():
                            log_s += ' {}: {:.4e}'.format(k, v)
                        logger.info(log_s)
                        if opt['use_tb_logger'] and 'debug' not in opt['name']:
            ...
            if current_step % opt['logger']['save_checkpoint_freq'] == 0:
                logger.info("Saving models and training states.current_step：",current_step)
...
if __name__ == '__main__':
    main()
    # m = torch.load("/root/group-video-proc/NAIC/mmsr_related/test_model_data/EDVR/EDVR_REDS_SR_L.pth")
    # print('a ')