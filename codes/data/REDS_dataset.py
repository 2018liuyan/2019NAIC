...
class REDSDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.paths_GT, self.path_LQ = self._get_key()
    ...
    def _get_key(self):
        ...
        with open(osp.join(self.opt['dataroot_GT'], 'key.txt'), 'rb') as f:
            for line in f:
                # print(line.decode('utf-8'))
                key = line.decode('utf-8').split('\n')[0]
                if key.split('_')[-1] == 'GT':
                    gt.append(key)
                else:
                    lr.append(key)
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            LQ_size_tuple = (3, 540, 960) if self.LR_input else (3, 180, 320)
            img_LQ_l = []
            # print(neighbor_list)
            for v in neighbor_list:
                img_LQ_path = osp.join(self.LQ_root, name_a, str(v))
                if self.data_type == 'mc':
                    if self.LR_input:
                        img_LQ = self._read_img_mc(img_LQ_path)
                    else:
                        img_LQ = self._read_img_mc_BGR(self.LQ_root, name_a, str(v))
                    img_LQ = img_LQ.astype(np.float32) / 255.
                elif self.data_type == 'lmdb':
                    img_LQ = util.read_img(self.LQ_env, name_a +'_'+ str(v), LQ_size_tuple)
                else:
            ...
            # print(len(img_LQ_l),img_LQ_l[0].shape)
            ...
            # print(key)
            return self.__getitem__(random.randrange(1, len(self.paths_GT) - 5))

        # print('key',key, img_GT.shape, img_LQs.shape)
